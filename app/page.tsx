...
export default function Home() {
  ...
  return <main className="overflow-hidden bg-[#f4eee5] text-[#302b27]">
    ...
    <section id="inicio" className="relative h-[100svh] min-h-[760px] text-white">
      <img src={asset('/portfolio/01.webp')} alt="Casal de noivos em um jardim" className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(28,23,20,.72),rgba(28,23,20,.2))]"/><div className="absolute inset-x-0 bottom-0 h-52 bg-gradient-to-t from-black/45 to-transparent"/>
    ...
    <section id="portfolio" className="bg-[#302b27] py-24 text-white md:py-32"><div className="mx-auto max-w-[1440px] px-5 md:px-10 lg:px-16">
      ...
      <div className="grid auto-rows-[240px] grid-cols-2 gap-2 md:auto-rows-[340px] md:grid-cols-4 md:gap-3">{photos.map(([src,alt],i)=><figure key={src} className={`group overflow-hidden ${i===0||i===4?'row-span-2':''} ${i===2?'md:col-span-2':''}`}><img src={asset(src)} alt={alt} loading={i>1?'lazy':undefined} className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.025]"/></figure>)}</div>
    ...
    <section id="experiencia" className="mx-auto max-w-[1320px] px-5 py-24 md:px-10 md:py-36 lg:px-16"><div className="grid gap-16 lg:grid-cols-[1fr_1.05fr] lg:items-center">
      <div className="relative pb-7 pr-7"><img src={asset('/portfolio/03.webp')} alt="Momento espontâneo de um casamento" className="relative z-10 aspect-[4/5] w-full object-cover"/><div className="absolute bottom-0 right-0 h-[65%] w-[65%] border border-[#ad755d]"/></div>
    ...
    <section className="relative min-h-[620px] text-white"><img src={asset('/portfolio/09.webp')} alt="Casal celebrando o casamento" className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-[#241f1b]/65"/><div className="relative mx-auto flex min-h-[620px] max-w-4xl flex-col items-center justify-center px-5 text-center"><Heart className="h-7 w-7"/><p className="eyebrow mt-6 text-white/65">Sua história merece espaço na agenda</p><h2 className="mt-5 font-serif text-[clamp(3rem,7vw,6.5rem)] leading-[.95] tracking-[-.05em]">Vamos guardar<br/><em>esse dia?</em></h2><p className="mt-7 max-w-lg leading-7 text-white/75">Conte a data e como vocês imaginam a celebração. A conversa começa sem compromisso, direto com a Luna.</p><div className="mt-9"><Cta inverse/></div></div></section>
